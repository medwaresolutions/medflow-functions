    ...
    for (const [key, value] of Object.entries(jsonRaw)) {
      if (value !== null && value !== '') {
        if (key.indexOf('q') === 0) {
          ...
          // console.log('index', key, value, index);
          if (index !== -1) {
            const keySliced = key.slice(index + 1);
            let match = false;
            for (let i = 0; i < filterList.length; i++) {
              console.log('keySliced', keySliced, filterList[i])
...
// To deploy 
// supabase functions deploy jotform-pb087 --no-verify-jwt